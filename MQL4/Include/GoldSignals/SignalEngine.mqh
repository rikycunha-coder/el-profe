...
//|                                                                   |
//|   Version MQL4: en MT4 no existen los handles de indicador, asi   |
//|   que los valores se leen directamente con iMA/iRSI/iATR/iADX.    |
//+------------------------------------------------------------------+
...
public:
   ...
   int               Loaded(void) { return m_loaded; }
   GSSettings        Settings(void) { return m_cfg; }
   int               MinBars(void);
   double            Ema(const int shift,const bool fast);
...
//| datos validos                                                     |
//+------------------------------------------------------------------+
int CGoldSignalEngine::MinBars(void)
  {
...
bool CGoldSignalEngine::Refresh(const int bars)
  {
   ...
   //--- en MQL4 los indicadores se consultan vela a vela
   if(ArrayResize(m_ema_f,want)!=want) return(false);
   if(ArrayResize(m_ema_s,want)!=want) return(false);
   if(ArrayResize(m_rsi,want)  !=want) return(false);
   if(ArrayResize(m_atr,want)  !=want) return(false);
   if(ArrayResize(m_adx,want)  !=want) return(false);
   ...
   for(int i=0;i<want;i++)
     {
      m_ema_f[i] = iMA(m_symbol,m_tf,m_cfg.ema_fast,0,MODE_EMA,PRICE_CLOSE,i);
      m_ema_s[i] = iMA(m_symbol,m_tf,m_cfg.ema_slow,0,MODE_EMA,PRICE_CLOSE,i);
      m_rsi[i]   = iRSI(m_symbol,m_tf,m_cfg.rsi_period,PRICE_CLOSE,i);
      m_atr[i]   = iATR(m_symbol,m_tf,m_cfg.atr_period,i);
      m_adx[i]   = iADX(m_symbol,m_tf,m_cfg.adx_period,PRICE_CLOSE,MODE_MAIN,i);
     }
...
double CGoldSignalEngine::TrendEmaAt(const datetime bar_time)
  {
   ...
   return(iMA(m_symbol,m_cfg.trend_tf,m_cfg.trend_ema,0,MODE_EMA,PRICE_CLOSE,htf_shift));
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
double CGoldSignalEngine::Ema(const int shift,const bool fast)
  {
...
//+------------------------------------------------------------------+
double CGoldSignalEngine::Atr(const int shift)
  {
   if(shift<0 || shift>=m_loaded)
      return(0.0);
   return(m_atr[shift]);
...
//+------------------------------------------------------------------+
double CGoldSignalEngine::High(const int shift)
  {
   if(shift<0 || shift>=m_loaded)
      return(0.0);
   return(m_high[shift]);
...
//+------------------------------------------------------------------+
double CGoldSignalEngine::Low(const int shift)
  {
   if(shift<0 || shift>=m_loaded)
      return(0.0);
   return(m_low[shift]);
...
bool CGoldSignalEngine::Evaluate(const int shift,GSResult &res)
  {
   ...
   if(shift<=1 && m_cfg.max_spread_points>0.0)
     {
      double spread_pts = MarketInfo(m_symbol,MODE_SPREAD);
