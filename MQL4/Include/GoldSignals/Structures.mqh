...
public:
   ...
   double            Open(const int s) { return((s>=0 && s<m_loaded) ? m_open[s]  : 0.0); }
   double            High(const int s) { return((s>=0 && s<m_loaded) ? m_high[s]  : 0.0); }
   double            Low(const int s) { return((s>=0 && s<m_loaded) ? m_low[s]   : 0.0); }
   double            Close(const int s) { return((s>=0 && s<m_loaded) ? m_close[s] : 0.0); }
   double            Atr(const int s) { return((s>=0 && s<m_loaded) ? m_atr[s]   : 0.0); }
   datetime          Time(const int s) { return((s>=0 && s<m_loaded) ? m_time[s]  : 0); }
   int               MinBars(void) { return(m_lookback+m_depth*2+30); }

   //--- estructura y swings
   MarketStructure   Structure(void) { return m_st; }
   int               SwingHighs(void) { return m_n_sh; }
   int               SwingLows(void) { return m_n_sl; }
   SwingPoint        SwingHigh(const int i);
   SwingPoint        SwingLow(const int i);

   double            HighLineAt(const int s);
   double            LowLineAt(const int s);
...
bool CPatternScanner::LoadData(const int bars)
  {
   ...
   //--- en MQL4 no hay CopyBuffer: el ATR se lee vela a vela
   if(ArrayResize(m_atr,want)!=want)
      return(false);
   ArraySetAsSeries(m_atr,true);
   for(int i=0;i<want;i++)
      m_atr[i] = iATR(m_symbol,m_tf,m_atr_period,i);
...
//+------------------------------------------------------------------+
SwingPoint CPatternScanner::SwingHigh(const int i)
  {
   SwingPoint empty;
...
//+------------------------------------------------------------------+
SwingPoint CPatternScanner::SwingLow(const int i)
  {
   SwingPoint empty;
   empty.shift = -1; empty.price = 0.0; empty.time = 0;
   if(i<0 || i>=m_n_sl)
...
//+------------------------------------------------------------------+
double CPatternScanner::HighLineAt(const int s)
  {
...
//+------------------------------------------------------------------+
double CPatternScanner::LowLineAt(const int s)
  {
