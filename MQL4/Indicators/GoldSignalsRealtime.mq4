...
int OnInit(void)
  {
   ...
   SetIndexStyle(1,DRAW_ARROW,EMPTY,2,clrOrangeRed);
   SetIndexArrow(0,233);
   SetIndexArrow(1,234);
   ...
   IndicatorShortName("Senales Oro");
...
void DrawLevels(const GSResult &r)
  {
   ObjectsDeleteAll(0,g_prefix+"lvl_",-1,-1);
...
void ShowPanel(void)
  {
   ...
   txt += StringFormat("Bid %s / Ask %s   spread %d pts\n",
                       DoubleToString(bid,_Digits),DoubleToString(ask,_Digits),spr);
