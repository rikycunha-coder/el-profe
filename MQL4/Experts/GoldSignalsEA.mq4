//+------------------------------------------------------------------+
//|                                                 GoldSignalsEA.mq4 |
//|   Robot de senales de ORO (XAUUSD) para MetaTrader 4.             |
//|                                                                   |
...
#property strict
#property description "Senales XAUUSD en tiempo real, con ejecucion opcional. Version MetaTrader 4."
...
input int              InpMagic          = 20260918;    // Numero magico
...
void Announce(const GSResult &r)
  {
   string head = StringFormat("%s %s %s | %s | score %d",
                              DirText(r.dir),_Symbol,TfName(Period()),
                              r.setup,r.score);
...
//+------------------------------------------------------------------+
//| En MT4 no hay "posicion": se recorren las ordenes abiertas        |
//+------------------------------------------------------------------+
bool HasOpenOrder(void)
  {
...
bool ValidStops(const ENUM_GS_DIR dir,const double entry,double &sl,double &tp)
  {
   double stops = MarketInfo(_Symbol,MODE_STOPLEVEL)*_Point;
   if(stops<=0.0)
      stops = 10*_Point;
...
void OpenTrade(const GSResult &r)
  {
   if(!IsTradeAllowed())
     {
      Print("El terminal no permite operar ahora mismo.");
      return;
     }
   if(HasOpenOrder())
     {
      Print("Ya hay una orden abierta del EA: se omite la entrada.");
      return;
     }
   ...
   int    cmd   = (r.dir==GS_BUY ? OP_BUY : OP_SELL);
   double entry = NormalizeDouble((r.dir==GS_BUY ? Ask : Bid),_Digits);
   double sl    = r.sl;
   ...
   string comment = StringFormat("GS s%d",r.score);

   ResetLastError();
   int ticket = OrderSend(_Symbol,cmd,lots,entry,InpSlippagePts,sl,tp,comment,InpMagic,0,
                          (r.dir==GS_BUY ? clrDodgerBlue : clrOrangeRed));

   //--- muchos brokers ECN rechazan SL/TP en la apertura (error 130):
   //--- se abre limpia y se ponen los niveles despues
   if(ticket<0 && GetLastError()==130)
     {
      ResetLastError();
      RefreshRates();
      entry  = NormalizeDouble((r.dir==GS_BUY ? Ask : Bid),_Digits);
      ticket = OrderSend(_Symbol,cmd,lots,entry,InpSlippagePts,0,0,comment,InpMagic,0,clrNONE);

      if(ticket>=0 && OrderSelect(ticket,SELECT_BY_TICKET))
        {
         double sl2 = r.sl;
         double tp2 = (InpUseTp2 ? r.tp2 : r.tp1);
         if(ValidStops(r.dir,OrderOpenPrice(),sl2,tp2))
            if(!OrderModify(ticket,OrderOpenPrice(),sl2,tp2,0,clrNONE))
               PrintFormat("Orden %d abierta sin SL/TP: OrderModify fallo (error %d)",
                           ticket,GetLastError());
        }
     }
   ...
   if(ticket>=0)
     {
      g_trades_today++;
      PrintFormat("Orden %d enviada: %s %.2f lotes  SL %s  TP %s",
                  ticket,DirText(r.dir),lots,
                  DoubleToString(sl,_Digits),DoubleToString(tp,_Digits));
     }
   else
      PrintFormat("Error al enviar la orden: %d",GetLastError());
  }
//+------------------------------------------------------------------+
//| Break-even y trailing sobre las ordenes abiertas                  |
//+------------------------------------------------------------------+
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
     {
      ...
      if(InpBreakEven)
        {
         ...
         if(is_buy  && price-open>=trigger && (sl<open || sl==0.0)) new_sl = open;
         if(!is_buy && open-price>=trigger && (sl>open || sl==0.0)) new_sl = open;
        }
      ...
      if(new_sl<=0.0 || new_sl==NormalizeDouble(sl,_Digits))
         continue;

      //--- no mover el stop dentro de la zona prohibida del broker
      double stops = MarketInfo(_Symbol,MODE_STOPLEVEL)*_Point;
      if(is_buy  && price-new_sl<stops) continue;
      if(!is_buy && new_sl-price<stops) continue;

      if(!OrderModify(OrderTicket(),open,new_sl,tp,0,clrNONE))
         PrintFormat("No se pudo mover el stop de la orden %d (error %d)",
