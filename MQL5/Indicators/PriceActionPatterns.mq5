...
int OnInit(void)
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"Patrones y figuras");
...
void DrawStructure(void)
  {
   ObjectsDeleteAll(0,g_prefix+"st_");
...
void DrawCandleTags(void)
  {
   ObjectsDeleteAll(0,g_prefix+"cd_");
...
void DrawSignalLevels(const PatternSignal &sig)
  {
   ObjectsDeleteAll(0,g_prefix+"lv_");
...
void Aviso(const string texto)
  {
   Comment("PATRONES Y FIGURAS - "+_Symbol+" "+EnumToString((ENUM_TIMEFRAMES)Period())+"\n"+texto);
